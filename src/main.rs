...
fn App() -> impl IntoView {
    ...
    Effect::new(move |_| {
        if velocity.get() > 0.1 {
            set_rotation.update(|r| *r += velocity.get());
            set_velocity.update(|v| *v *= 0.95); // Friction

            request_animation_frame(move || {});
        } else if is_spinning.get() && velocity.get() <= 0.1 {
            ...
            if !entries.is_empty() {
                ...
                let normalized_angle = (360.0 - (final_angle % 360.0) + 90.0) % 360.0;
                let selected_idx = (normalized_angle / segment_angle) as usize % entries.len();
    ...
    let spin_wheel = move |_| {
        ...
        // Random initial velocity for 5-8 full rotations
        let spins = 5.0 + (rand::random::<f64>() * 3.0);
        let initial_velocity = spins * 360.0 / 60.0; // Spread over ~60 frames
        ...
        <div>
            <div class="controls">
                ...
                <select
                    ...
                    style="padding: 0.75rem; background: #3a3a3a; color: #fff; border: none; border-radius: 6px; cursor: pointer;"
                >
